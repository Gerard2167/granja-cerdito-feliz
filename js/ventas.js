...
const getNewInvoiceNumber = async () => {
    try {
        const response = await fetch(`${API_URL_SEQUENCE}/invoiceNumber/increment`, {
            method: 'POST',
        });
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
...
formVenta.addEventListener('submit', async (e) => {
    ...
    try {
        let response;
        if (editSaleId === null) {
            response = await fetch(API_URL_VENTAS, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(ventaData),
            });
        } else {
            response = await fetch(`${API_URL_VENTAS}/${editSaleId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(ventaData),
            });
...
tablaVentasBody.addEventListener('click', async (e) => {
    if (e.target.classList.contains('btn-eliminar')) {
        ...
        if (confirm('¿Estás seguro de que quieres eliminar esta venta?')) {
            try {
                const response = await fetch(`${API_URL_VENTAS}/${saleId}`, {
                    method: 'DELETE',
                });
                if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
                await renderizarTabla();
