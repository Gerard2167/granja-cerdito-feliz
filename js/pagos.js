...
const getNewReceiptNumber = async () => {
    try {
        const response = await fetch(`${API_URL_SEQUENCE}/receiptNumber/increment`, {
            method: 'POST',
        });
...
formPago.addEventListener('submit', async (e) => {
    ...
    try {
        let response;
        if (editPagoId === null) {
            response = await fetch(API_URL_PAGOS, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(pagoData),
            });
        } else {
            response = await fetch(`${API_URL_PAGOS}/${editPagoId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(pagoData),
            });
...
tablaPagosBody.addEventListener('click', async (e) => {
    if (e.target.classList.contains('btn-eliminar')) {
        ...
        if (confirm('¿Estás seguro de que quieres eliminar este pago?')) {
            try {
                const response = await fetch(`${API_URL_PAGOS}/${pagoId}`, {
                    method: 'DELETE',
                });
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
