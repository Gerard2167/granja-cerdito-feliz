...
formColaborador.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (editColaboradorId === null) {
            response = await window.fetchWithAuth(API_URL_COLABORADORES, {
                method: 'POST',
                body: JSON.stringify(colaboradorData),
            });
        } else {
            response = await window.fetchWithAuth(`${API_URL_COLABORADORES}/${editColaboradorId}`, {
                method: 'PUT',
                body: JSON.stringify(colaboradorData),
            });
        }
...
tablaColaboradoresBody.addEventListener('click', async (e) => {
    if (e.target.classList.contains('btn-eliminar')) {
        ...
        if (confirm('¿Estás seguro de que quieres eliminar este colaborador?')) {
            try {
                const response = await window.fetchWithAuth(`${API_URL_COLABORADORES}/${colaboradorId}`, {
                    method: 'DELETE',
                });
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
