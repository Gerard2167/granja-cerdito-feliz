...
const API_URL_PROVEEDORES = '/proveedores';
...
const fetchProveedores = async () => {
    try {
        const response = await window.fetchWithAuth(API_URL_PROVEEDORES);
        if (!response.ok) {
...
formProveedor.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (editProveedorId === null) {
            response = await window.fetchWithAuth(API_URL_PROVEEDORES, {
                method: 'POST',
                body: JSON.stringify(proveedorData),
            });
        } else {
            response = await window.fetchWithAuth(`${API_URL_PROVEEDORES}/${editProveedorId}`, {
                method: 'PUT',
                body: JSON.stringify(proveedorData),
            });
        }
...
tablaProveedoresBody.addEventListener('click', async (e) => {
    if (e.target.classList.contains('btn-eliminar')) {
        ...
        if (confirm('¿Estás seguro de que quieres eliminar este proveedor?')) {
            try {
                const response = await window.fetchWithAuth(`${API_URL_PROVEEDORES}/${proveedorId}`, {
                    method: 'DELETE',
                });
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
