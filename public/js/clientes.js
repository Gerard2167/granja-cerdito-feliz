...
// Este script se ejecuta cuando se carga la página de clientes.
    ...
    const fetchClientes = async () => {
        try {
            const response = await window.fetchWithAuth(API_URL);
            if (!response.ok) {
    ...
    formCliente.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (editClientId === null) {
                // Modo Agregar
                response = await window.fetchWithAuth(API_URL, {
                    method: 'POST',
                    body: JSON.stringify(clienteData),
                });
            } else {
                // Modo Editar
                response = await window.fetchWithAuth(`${API_URL}/${editClientId}`, {
                    method: 'PUT',
                    body: JSON.stringify(clienteData),
                });
            }
    ...
    tablaClientesBody.addEventListener('click', async (e) => {
        if (e.target.classList.contains('btn-eliminar')) {
            ...
            if (confirm('¿Estás seguro de que quieres eliminar este cliente?')) {
                try {
                    const response = await window.fetchWithAuth(`${API_URL}/${clientId}`, {
                        method: 'DELETE',
                    });
                    if (!response.ok) {
                        const errorData = await response.json().catch(() => ({}));
