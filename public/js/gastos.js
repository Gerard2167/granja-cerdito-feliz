...
const fetchGastos = async () => {
    try {
        const response = await window.fetchWithAuth(API_URL_GASTOS);
        if (!response.ok) {
...
formGasto.addEventListener('submit', async (e) => {
    ...
    try {
        let response;
        if (editGastoId === null) {
            response = await window.fetchWithAuth(API_URL_GASTOS, {
                method: 'POST',
                body: JSON.stringify(gastoData),
            });
        } else {
            response = await window.fetchWithAuth(`${API_URL_GASTOS}/${editGastoId}`, {
                method: 'PUT',
                body: JSON.stringify(gastoData),
            });
        }
...
tablaGastosBody.addEventListener('click', async (e) => {
    if (e.target.classList.contains('btn-eliminar')) {
        ...
        if (confirm('¿Estás seguro de que quieres eliminar este gasto?')) {
            try {
                const response = await window.fetchWithAuth(`${API_URL_GASTOS}/${gastoId}`, {
                    method: 'DELETE',
                });
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
